...
flags.DEFINE_integer("output_width", 96, "The size of the output images to produce. If None, same value as output_height [None]")
flags.DEFINE_string("dataset", "MIAS", "The name of dataset [celebA, mnist, lsun]")
flags.DEFINE_string("input_fname_pattern", "*.jpg", "Glob pattern of filename of input images [*]")
flags.DEFINE_string("checkpoint_dir", "checkpointMias", "Directory name to save the checkpoints [checkpoint]")
...
def main(_):
	...
	#data = readFaces()
	options = {"cropping": "seq", "crop_step": 8 , "crop_skip": 0.031, "crop_size": (FLAGS.input_width, FLAGS.input_width)}
	mias = MIAS(data, None, [1,0,0],False, ["NORM"], options )
	GANoptions = {'noise size' : 100, 'noise type' : 'normal', 'label smooth':True}
	##########AYAD###################################
	print("entering sesh")
	with tf.Session(config=run_config) as sess:
		...
		#query_image = 'm.png'
		#generate_latent_for_query(sess, dcgan, query_image,inputClass=mias , FLAGS=FLAGS,OPTION= OPTION)
